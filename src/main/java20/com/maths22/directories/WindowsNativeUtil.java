...
final class WindowsNativeUtil {
  ...
  static boolean isSupported() {
    try {
      ...
      if(!"disabled".equals(System.getProperty(POWERSHELL_WARNING_PROPERTY))) {
        System.err.println("WARN: Native access not available, falling back to less reliable methods. " +
  ...
  static String getWinDir(UUID uuid) {
    ...
    try (var allocator = Arena.openConfined()) {
      ...
      int invokeRes = (int) shGetKnownFolderPath.invokeExact(guid, 0, MemorySegment.NULL, retVal);
      if(invokeRes != 0) {
        return null;
      }
      MemorySegment returnedString = retVal.get(ValueLayout.ADDRESS.asUnbounded(), 0);
      int offset;
      ...
      MemorySegment segment = MemorySegment.ofAddress(returnedString.address(), offset, allocator.scope());
